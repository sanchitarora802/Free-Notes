...
function Notes(props) {
  ...
  const handleSubmit = (e) => {
    ...
    context.editNote(
      ...
      demoNote.description.toString(),
      demoNote.tag.toString()
    );
    handleClose();
  };
      ...
      <div className="container">
        <div className="row my-3">
          <div className={`container mx-2 text-${props.textheadingcolor} `}>
            {Array.isArray(context.notes) && context.notes.length === 0
              ? "No Notes To Display"
              : ""}
          </div>
          ...
          {Array.isArray(context.notes) &&
            context.notes.map((note) => {
              return (
                <NoteItem
                  mode={props.mode}
                  textheadingcolor={props.textheadingcolor}
                  key={note._id}
                  note={note}
                  openModal={openModal}
                />
